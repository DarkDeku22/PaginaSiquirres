  <!-- MODAL DE EDITAR ACTIVIDAD PRINCIPAL -->
            <div class="modal fade" id="editarPaginaNosotros{{ $paginaI[0]->id_pagina }}">
                <div class="modal-dialog modal-dialog-centered modal-None modal-dialog-scrollable">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title colorTitulos">
                                Configurar Datos del encabezado, sección de <strong>{{ $paginaI[0]->titulo }} </strong></h5>
                            <button class="btn btn-sm btn-outline-danger" data-bs-dismiss="modal">X</button>
                        </div>
                        <div class="modal-body">

                            <form action="{{ route('nosotrosPrincipal', $paginaI[0]->id_pagina) }}" method="POST">
                                @method('put')
                                @csrf
                                ...
                                <div class="px-3 py-2">
                                    ...
                                    <div class="">
                                        ...
                                        <input type="hidden" name="id_padre" value="{{ $paginaI[0]->id_pagina }}">

                                        <input type="text" class="form-control colorImput" name="tituActividad"
                                            maxlength="100" value="{{ $paginaI[0]->titulo }}" placeholder="Escriba el título"
                                            required>
                                    </div>
                                    <div class="mt-3">
                                    ...
                                    <div class="mb-4">
                                        <div>
                                            <textarea class="form-control colorImput" id="" rows="10" name="tituDes"
                                                placeholder="Escriba la información">{{ $paginaI[0]->descripcion }}</textarea>
                                        </div>
                                    </div>
